...
using PaymentMockService.Messaging;
using Serilog;
...
builder.Host.UseSerilog((context, loggerConfiguration) =>
{
    ...
    loggerConfiguration
        ...
        .Enrich.WithProperty("Service", "PaymentMockService")
        .WriteTo.Console()
        .WriteTo.Elasticsearch(new ElasticsearchSinkOptions(new Uri(elasticUri))
        {
            AutoRegisterTemplate = true,
            IndexFormat = "paymentservice-logs-{0:yyyy.MM}"
        });
});
...
builder.Services.AddSingleton<PaymentResultPublisher>();
...
builder.Services
    ...
    .WithTracing(tracerProviderBuilder =>
    {
        tracerProviderBuilder
            .SetResourceBuilder(ResourceBuilder.CreateDefault().AddService("PaymentMockService"))
